...
### Automatic League Backups

The host server now backs up shared league state automatically.

- Backup folder: `league_backup/`
- Schedule: every day at midnight (host machine local time)
- File name format: `league-state-YYYY-MM-DD.json`

An initial backup is also created when the server starts.
...
Required Node.js version for this project:

- Node.js 20.19.0+ (or 22.12.0+)

Vite 8 does not support Node 18. If you see an error like "CustomEvent is not defined" from vite/dist/node/cli.js, switch to a supported Node version.
...
Then install and use a supported Node version:
