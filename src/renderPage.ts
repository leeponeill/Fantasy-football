import './style.css'
import { getCurrentUsername, getPasswordResetRequests, requireCurrentUserTeamName, signOut } from './auth'
...
export function renderPage(title: string, activeTab: Tab, content = ''): void {
  ...
  const updateSettingsAlertState = (): void => {
    ...
    settingsLink.classList.toggle('settings-link--alert', hasPendingPasswordResetRequests)
    settingsLink.title = hasPendingPasswordResetRequests
      ? 'Settings (password reset request pending)'
      : 'Settings'
  }
