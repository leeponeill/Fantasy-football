...
export function renderPage(title: string, activeTab: Tab, content = ''): void {
  ...
  const isLee = username?.toLowerCase() === 'lee'
  const updateSettingsAlertState = (): void => {
    if (!settingsLink) {
      return
    }

    const hasPendingPasswordResetRequests = isLee && getPasswordResetRequests().length > 0
    const hasUnreadAdminMessages = hasUnreadAdminNotification()
    const shouldAlert = hasPendingPasswordResetRequests || hasUnreadAdminMessages
    settingsLink.classList.toggle('settings-link--alert', shouldAlert)

    if (hasPendingPasswordResetRequests && hasUnreadAdminMessages) {
      settingsLink.title = 'Settings (password reset request pending, unread admin message)'
      return
    }

    if (hasPendingPasswordResetRequests) {
      settingsLink.title = 'Settings (password reset request pending)'
      return
    }

    settingsLink.title = hasUnreadAdminMessages
      ? 'Settings (unread admin message)'
      : 'Settings'
  }

  updateSettingsAlertState()
  window.addEventListener(sharedLeagueUpdatedEvent, updateSettingsAlertState)
