...
// Keys currently being sent to the server in an in-flight flush request.
// Prevents stale poll responses from overwriting locally-pending state.
...
function applySharedStorageSnapshot(snapshot: SharedStorageSnapshot): boolean {
  ...
  try {
    ...
    for (const key of existingSharedKeys) {
      if (!(key in snapshot) && !pendingSet.has(key) && !inFlightKeys.has(key)) {
    ...
    for (const [key, value] of Object.entries(snapshot)) {
      // Skip keys with pending or in-flight local writes — local state is more recent.
      if (pendingSet.has(key) || inFlightKeys.has(key)) {
        continue
      }
...
export async function flushSharedLeagueStorage(): Promise<void> {
  ...
  flushPromise = (async () => {
    try {
      ...
      // Clear in-flight tracking before applying so the authoritative server
      // response is written back without being blocked by our own keys.
      inFlightKeys.clear()
