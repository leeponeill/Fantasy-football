...
function hasUserSeenNotification(notification: AdminNotification, username: string): boolean {
  return notification.seenBy.some((seenUsername) => seenUsername.toLowerCase() === username.toLowerCase())
}

function renderNotificationMarkup(notifications: AdminNotification[], username: string): string {
  if (notifications.length === 0) {
    return '<p class="players-help">No admin messages right now.</p>'
  }

  return notifications
    .map((notification) => {
      const hasSeen = hasUserSeenNotification(notification, username)
      return `
      <div class="admin-notification-panel" data-notification-id="${escapeHtml(notification.id)}">
        <div class="admin-user-meta">
          <strong>Message from ${escapeHtml(notification.senderUsername)}</strong>
        ...
        <div class="admin-user-actions">
          <button
            type="button"
            class="lock-team-btn"
            data-action="confirm-notification-seen"
            data-notification-id="${escapeHtml(notification.id)}"
            ${hasSeen ? 'disabled' : ''}
          >
            ${hasSeen ? 'Seen' : 'Confirm Seen'}
          </button>
        </div>
      </div>
    `
...
function renderSettings(theme: ThemeMode): void {
  ...
  const markup = `
    <section class="admin-card settings-card" id="admin-notification-section" ${currentNotifications.length > 0 ? '' : 'hidden'}>
      <h2>Admin Messages</h2>
      <div id="admin-notification-view">${renderNotificationMarkup(currentNotifications, username)}</div>
    </section>
  ...
  const adminNotificationSection = document.querySelector<HTMLElement>('#admin-notification-section')
  const adminNotificationView = document.querySelector<HTMLDivElement>('#admin-notification-view')
  ...
  const syncAdminNotificationView = (): void => {
    if (!adminNotificationView || !adminNotificationSection) {
      return
    }

    const notifications = getAdminNotifications()
    adminNotificationSection.hidden = notifications.length === 0
    adminNotificationView.innerHTML = renderNotificationMarkup(notifications, getCurrentUsername() ?? '')
  }

  if (adminNotificationView) {
    adminNotificationView.addEventListener('click', async (event) => {
      const target = event.target as HTMLElement
      const confirmButton = target.closest<HTMLButtonElement>('[data-action="confirm-notification-seen"]')
      if (!confirmButton) {
        return
      }
      ...
      if (!currentUsername || !notificationId) {
        return
      }

      const result = markAdminNotificationSeen(currentUsername, notificationId)
      if (!result.ok) {
        return
      }

      await flushSharedLeagueStorage()
      syncAdminNotificationView()
    })
  }
