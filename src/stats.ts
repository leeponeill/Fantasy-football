...
function getFixtureMatchdayByGameweek(gameweekNumber: number): WCFixtureMatchday | null {
  ...
  const groupedMatchdays = fixtureMatchdays.filter((matchday) => getRoundGameweekNumber(matchday.round) === normalized)
  if (groupedMatchdays.length > 0) {
    return {
      matchday: normalized,
      round: `Group Stage - Matchday ${normalized}`,
      games: groupedMatchdays.flatMap((matchday) => matchday.games),
    }
  }

  const index = normalized - 1
  if (index < 0 || index >= fixtureMatchdays.length) {
    return null
  }

  return fixtureMatchdays[index] ?? null
}
