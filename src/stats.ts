...
type ImportedPlayerStat = {
  ...
  penaltyShootoutGoalsConceded: number
  shotSaves: number
  yellowCards: number
  redCards: number
...
const teamAliases: Record<string, string> = {
  ...
  // WC 2026 canonical team names → themselves (identity mappings)
  canada: 'canada',
  mexico: 'mexico',
  ...
  <section class="stats-container">
    ...
    <section class="performance-panel" style="margin-bottom: 1.5rem;">
      ...
      <div class="performance-form" style="grid-template-columns: 1fr;">
        ...
        <div class="form-group">
          <label for="individual-game-select">Individual Game</label>
          <select id="individual-game-select">
            <option value="">Select individual game</option>
          </select>
        </div>
        <button id="clear-individual-game-btn" type="button" class="award-btn" disabled>Clear Selected Game Points</button>
        <button id="scan-individual-game-btn" type="button" class="calculate-btn" disabled>Scan Selected Game</button>
        <div class="form-group">
    ...
    <div class="stats-grid">
      ...
      <div class="performance-panel">
        ...
        <form id="performance-form" class="performance-form">
          ...
          <div class="form-group">
            <label for="penalty-shootout-goals-conceded">Penalty Shootout Goals Conceded:</label>
            <input id="penalty-shootout-goals-conceded" type="number" min="0" value="0" />
          </div>
...
const individualGameSelect = document.querySelector<HTMLSelectElement>('#individual-game-select')
const clearIndividualGameBtn = document.querySelector<HTMLButtonElement>('#clear-individual-game-btn')
const scanIndividualGameBtn = document.querySelector<HTMLButtonElement>('#scan-individual-game-btn')
...
const individualFixtureGameByKey = new Map<string, WCFixtureGame>()
...
function readImportedMatchPlayerPoints(): Record<string, Record<string, number>> {
  const raw = getSharedItem(importedMatchPlayerPointsStorageKey)
  if (!raw) {
    return {}
  }
  ...
  try {
    ...
    for (const [eventId, contributionMap] of Object.entries(parsed)) {
      if (typeof eventId !== 'string' || !contributionMap || typeof contributionMap !== 'object' || Array.isArray(contributionMap)) {
        continue
      }

      const sanitizedContributions: Record<string, number> = {}
      for (const [playerKey, points] of Object.entries(contributionMap as Record<string, unknown>)) {
        if (typeof playerKey !== 'string' || typeof points !== 'number' || !Number.isFinite(points)) {
          continue
        }
        sanitizedContributions[playerKey] = points
      }

      if (Object.keys(sanitizedContributions).length > 0) {
        next[eventId] = sanitizedContributions
      }
    }
...
function saveImportedMatchPlayerPoints(pointsByEventId: Record<string, Record<string, number>>): void {
  setSharedItem(importedMatchPlayerPointsStorageKey, JSON.stringify(pointsByEventId))
}

function readAutoScannedFixtureKeys(): Set<string> {
  const raw = getSharedItem(autoScannedFixtureKeysStorageKey)
  if (!raw) {
    return new Set()
  }
...
function saveAutoScannedFixtureKeys(keys: Set<string>): void {
  setSharedItem(autoScannedFixtureKeysStorageKey, JSON.stringify(Array.from(keys).sort()))
}
...
function readStoredFixtureResults(): StoredFixtureResult[] {
  const raw = getSharedItem(fixtureResultsStorageKey)
  if (!raw) {
    return []
  }
  ...
  try {
    ...
    return parsed.filter((value): value is StoredFixtureResult => {
...
function saveStoredFixtureResults(results: StoredFixtureResult[]): void {
  setSharedItem(fixtureResultsStorageKey, JSON.stringify(results))
}
...
function applyImportedMatchPointsForEvent(eventId: string, calculatedRows: CalculatedImportRow[]): number {
  const pointsByEventId = readImportedMatchPlayerPoints()
  const previousContributions = pointsByEventId[eventId] ?? {}
  const nextContributions: Record<string, number> = {}

  for (const row of calculatedRows) {
    const playerKey = `${row.matchedPlayer.team}::${row.matchedPlayer.name}`
    nextContributions[playerKey] = row.points
  }

  const affectedPlayerKeys = new Set<string>([
    ...Object.keys(previousContributions),
    ...Object.keys(nextContributions),
  ])

  let replacedValuesCount = 0
  for (const playerKey of affectedPlayerKeys) {
    const previousPoints = previousContributions[playerKey] ?? 0
    const nextPoints = nextContributions[playerKey] ?? 0
    if (previousPoints === nextPoints) {
      continue
    }

    const keyParts = playerKey.split('::')
    if (keyParts.length < 2) {
      continue
    }

    const teamName = keyParts[0]
    const playerName = keyParts.slice(1).join('::')
    const currentPoints = getPlayerPoints(playerName, teamName)
    updatePlayerPoints(playerName, teamName, currentPoints - previousPoints + nextPoints)
  ...
  if (Object.keys(nextContributions).length > 0) {
    pointsByEventId[eventId] = nextContributions
  } else {
    delete pointsByEventId[eventId]
  }

  const latestPointsByEventId = readImportedMatchPlayerPoints()
  if (Object.keys(nextContributions).length > 0) {
    latestPointsByEventId[eventId] = nextContributions
  } else {
    delete latestPointsByEventId[eventId]
  }

  saveImportedMatchPlayerPoints(latestPointsByEventId)
...
function parseFixtureKickoff(game: WCFixtureGame, now: Date): Date | null {
  ...
  const normalizedTime = game.time.trim().toLowerCase()
  ...
  if (amPmMatch) {
    ...
    minute = Number.parseInt(amPmMatch[2] ?? '0', 10)
...
function setAutoScanButtonsDisabled(disabled: boolean): void {
  ...
  if (clearIndividualGameBtn) {
    clearIndividualGameBtn.disabled = disabled || !individualGameSelect?.value
  }

  if (scanIndividualGameBtn) {
    scanIndividualGameBtn.disabled = disabled || !individualGameSelect?.value
  }
...
function getAssignedGameweekNumber(
  ...
  // Keep the existing GW4 group-stage games and extend GW4 through Colombia vs Ghana.
...
function getFixtureGameweekByGameweek(gameweekNumber: number): WCFixtureGameweek | null {
  ...
  const round = roundLabels.size === 1
    ? Array.from(roundLabels)[0]
...
function getIndividualFixtureKey(game: WCFixtureGame): string {
  return `${game.date}::${game.time}::${game.match}`
}

function renderIndividualGameOptions(): void {
  if (!individualGameSelect) {
    return
  }

  const previousValue = individualGameSelect.value
  ...
  for (const [index, Gameweek] of fixtureGameweeks.entries()) {
    for (const game of Gameweek.games) {
      ...
      options.push(
        `<option value="${escapeHtml(fixtureKey)}">GW ${gameweekNumber} - ${escapeHtml(game.date)} ${escapeHtml(game.time)} - ${escapeHtml(game.match)}</option>`,
      )
    }
  }

  individualGameSelect.innerHTML = '<option value="">Select individual game</option>' + options.join('')

  if (previousValue && individualFixtureGameByKey.has(previousValue)) {
    individualGameSelect.value = previousValue
  }

  if (scanIndividualGameBtn) {
    scanIndividualGameBtn.disabled = !individualGameSelect.value
  }
}
...
function calculatePointsForForm(): void {
  ...
  const penaltyShootoutGoalsConceded = parseInt(performanceForm.querySelector<HTMLInputElement>('#penalty-shootout-goals-conceded')?.value ?? '0', 10)
...
async function importAndApplyMatchStats(): Promise<void> {
  ...
  try {
    ...
    applyImportedMatchPointsForEvent(eventId, calculated)
  ...
  try {
    for (const game of games) {
      ...
      try {
        ...
        replacedValuesCount += applyImportedMatchPointsForEvent(bestMatch.idEvent, calculated)
...
async function scanSelectedIndividualGameAndImport(): Promise<void> {
  if (!individualGameSelect) {
    return
  }

  const selectedKey = individualGameSelect.value.trim()
  if (!selectedKey) {
    setAutoImportMessage('Select an individual game first.', 'error')
    return
  }

  const selectedGame = individualFixtureGameByKey.get(selectedKey)
  if (!selectedGame) {
    setAutoImportMessage('Selected game was not found in fixtures.', 'error')
    return
  }

  await scanFixtureGamesAndImport(
    [selectedGame],
    `Individual game scan (${selectedGame.match})`,
    {
      skipAlreadyImported: false,
      includeZeroPoints: true,
      replaceExistingValues: true,
...
async function clearSelectedIndividualGamePoints(): Promise<void> {
  if (!individualGameSelect) {
    return
  }
  ...
  const confirmed = window.confirm(`Clear imported points for ${selectedGame.match}?`)
  if (!confirmed) {
    return
  }
  ...
  isAutoScanRunning = true
  setAutoScanButtonsDisabled(true)
  setAutoImportMessage(`Clearing imported points for ${selectedGame.match}...`, 'info')

  try {
    const queries = getFixtureSearchQueries(selectedGame.match)
    let matches: MatchSearchResult[] = []
    ...
    const bestMatch = selectBestMatchForFixture(selectedGame, matches)
    if (!bestMatch) {
      setAutoImportMessage(`Unable to find a finished match for ${selectedGame.match}.`, 'error')
      return
    }

    const replacedValuesCount = applyImportedMatchPointsForEvent(bestMatch.idEvent, [])

    const importedEventIds = readAutoImportedEventIds()
    importedEventIds.delete(bestMatch.idEvent)
    saveAutoImportedEventIds(importedEventIds)

    const scannedFixtureKeys = readAutoScannedFixtureKeys()
    const selectedFixtureKey = `${selectedGame.date}|${selectedGame.time}||${selectedGame.match}`
    ...
    if (selectedGameweek !== null) {
      ...
      saveStoredFixtureResults(storedResults)
    }

    await flushSharedLeagueStorage()
    renderAutoImportPreview([], 0)
    setAutoImportMessage(
      `Cleared imported points for ${selectedGame.match}. Replaced values: ${replacedValuesCount}.`,
      'ok',
    )
  } catch {
    setAutoImportMessage(`Unable to clear imported points for ${selectedGame.match}.`, 'error')
  } finally {
    isAutoScanRunning = false
    setAutoScanButtonsDisabled(false)
  }
...
function scheduleOneTimeGameweekScans(now: Date): void {
  ...
  for (const gameweekNumber of getAvailableGameweekNumbers()) {
    if (scheduledAutoScanGameweekKeys.has(gameweekNumber)) {
      continue
    }
    ...
    scheduledAutoScanGameweekKeys.add(gameweekNumber)
...
if (individualGameSelect) {
  individualGameSelect.addEventListener('change', () => {
    if (clearIndividualGameBtn && !isAutoScanRunning) {
      clearIndividualGameBtn.disabled = !individualGameSelect.value
    }

    if (scanIndividualGameBtn && !isAutoScanRunning) {
      scanIndividualGameBtn.disabled = !individualGameSelect.value
    }
  })
}
...
if (clearIndividualGameBtn) {
  clearIndividualGameBtn.addEventListener('click', () => {
    void clearSelectedIndividualGamePoints()
  })
}

if (scanIndividualGameBtn) {
  scanIndividualGameBtn.addEventListener('click', () => {
    void scanSelectedIndividualGameAndImport()
  })
}
