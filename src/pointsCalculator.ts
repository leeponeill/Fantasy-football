...
export function calculatePlayerPoints(performance: PlayerPerformance): { points: number; breakdown: Record<string, number> } {
  ...
  // Goals conceded (goalkeeper & defender only, -1 per 2 goals, excludes penalty shootout goals)
  if (performance.position === 'Goalkeeper' || performance.position === 'Defender') {
    const regularGoalsConceded = Math.max(0, performance.goalsConceded - performance.penaltyShootoutGoalsConceded)
    const concededPoints = Math.floor(regularGoalsConceded / 2) * -1
    if (concededPoints < 0) {
